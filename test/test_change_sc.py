...
from pySparkUtils.utils import change_sc
import pytest
pytestmark = pytest.mark.usefixtures("eng")

def test_no_input():
    with pytest.raises(ValueError) as excinfo:
        change_sc(sc=None, master=None)
    assert 'Both master and sc are None' in str(excinfo.value)

def test_cores(eng):
    n_cores = eng.defaultParallelism
    new_sc = change_sc(sc=eng)
    assert new_sc.defaultParallelism == n_cores
    new_sc.stop()

def test_local(eng):
    eng.stop()
    new_sc = change_sc(sc=None, master='local[2]', fail_on_timeout=False)
...
def test_args(eng):
    ...
    new_sc = change_sc(sc=eng, spark_rpc_message_maxSize=new_value)
